...
def plot_trajectory(cam_centers, output_path=None):
    """
    Plot the camera trajectory in 2D (top-down view, X-Y plane)
    Simple and clean style
    
    Args:
        cam_centers: List of camera center positions (N x 3)
    ...
    # Plot simple trajectory line
    ax.plot(x, y, '-', color='green', linewidth=2, label='Trajectory')
    
    # Mark start and end points (simple style)
    ax.plot(x[0], y[0], 'o', color='green', markersize=8, label='Start')
    ax.plot(x[-1], y[-1], 's', color='green', markersize=8, label='End')
    
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_title('3D Trajectory in 2D')
    ax.legend()
...
def visualize(scene_path, cfg):
    ...
    if cfg['visualize_cams']:
        ...
        # Plot and save trajectory using matplotlib
        if cfg.get('plot_trajectory', True):
            # Determine output path for trajectory plot
            output_dir = os.path.dirname(scene_path)
            traj_output_path = os.path.join(output_dir, "trajectory_plot.png")
            print(f"Plotting trajectory with {len(cam_centers)} camera poses...")
            plot_trajectory(cam_centers, output_path=traj_output_path)
        
        # Initialize Camera Trajectory in Open3D (make it more visible)
        num_lines = [1]
