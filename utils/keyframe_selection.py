...
import torch
import numpy as np
from utils.point_smoothing import get_smoothed_attribute
...
def keyframe_selection_overlap(gt_depth, w2c, intrinsics, keyframe_list, k, pixels=1600, config=None):
        """
        ...
        Args:
            ...
            config (dict, optional): configuration dictionary containing point_smoothing parameters.
        Returns:
        ...
        # 获取平滑参数配置
        if config is not None and 'point_smoothing' in config:
            smoothing_cfg = config['point_smoothing']
            use_adaptive_k = smoothing_cfg.get('use_adaptive_k', True)
            base_k = smoothing_cfg.get('base_k', 6)
            min_k = smoothing_cfg.get('min_k', 2)
            max_k = smoothing_cfg.get('max_k', 16)
            dropout = smoothing_cfg.get('dropout', 0.4)
            multi_scale = smoothing_cfg.get('multi_scale', False)
        else:
            # 默认使用自适应K值
            use_adaptive_k = True
            base_k = 6
        ...
        # 应用点云平滑处理
        pts = get_smoothed_attribute(
            pts, pts, 
            K=base_k, dropout=dropout,
