...
def params2rendervar(params, config=None):
    ...
    # 对点云属性进行平滑处理
    means3D = get_smoothed_attribute(
        params['means3D'], params['means3D'], 
        K=base_k, dropout=dropout,
        use_adaptive_k=use_adaptive_k,
        multi_scale=multi_scale,
        min_k=min_k, max_k=max_k
    )
    rgb_colors = get_smoothed_attribute(
        params['rgb_colors'], params['means3D'], 
        K=base_k, dropout=dropout,
        use_adaptive_k=use_adaptive_k,
        multi_scale=multi_scale,
        min_k=min_k, max_k=max_k
