...
def evaluate_ate(gt_traj, est_traj, return_aligned=False):
    """
    Compute ATE (Absolute Trajectory Error) RMSE between ground truth and estimated trajectories.
    
    Input : 
        gt_traj: list of 4x4 matrices 
        est_traj: list of 4x4 matrices
        len(gt_traj) == len(est_traj)
        return_aligned: if True, return both unaligned and aligned ATE RMSE
    
    Returns:
        if return_aligned=False: avg_trans_error (float) - aligned ATE RMSE
        if return_aligned=True: (ate_rmse_unaligned, ate_rmse_aligned) - both metrics
    """
    ...
    # Compute unaligned ATE RMSE (before alignment)
    if return_aligned:
        # Direct distance without alignment
        unaligned_error = np.linalg.norm(est_traj_pts.T - gt_traj_pts.T, axis=1)
        ate_rmse_unaligned = np.sqrt(np.mean(unaligned_error ** 2))
    
    # Align trajectories and compute aligned ATE RMSE
    _, _, trans_error = align(gt_traj_pts, est_traj_pts)
    # Fixed: Use RMSE instead of mean for consistency with trajectory_evaluation
    ate_rmse_aligned = np.sqrt(np.mean(trans_error ** 2))

    if return_aligned:
        return ate_rmse_unaligned, ate_rmse_aligned
    else:
        return ate_rmse_aligned
...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    if i % every_i == 0 or i == 1:
        ...
        if tracking:
            ...
            # Calculate ATE RMSE (both unaligned and aligned)
            ate_rmse_unaligned, ate_rmse_aligned = evaluate_ate(gt_w2c_list, latest_est_w2c_list, return_aligned=True)
            ate_rmse_unaligned = np.round(ate_rmse_unaligned, decimals=6)
            ate_rmse_aligned = np.round(ate_rmse_aligned, decimals=6)
            if wandb_run is not None:
                tracking_log = {f"{stage}/Latest Pose Error":iter_pt_error, 
                               f"{stage}/Latest Relative Pose Error":rel_pt_error,
                               f"{stage}/ATE RMSE (unaligned)":ate_rmse_unaligned,
                               f"{stage}/ATE RMSE (aligned)":ate_rmse_aligned}
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    try:
        ...
        # Convert w2c to c2w for proper trajectory comparison
        ...
        valid_gt_c2w_list = [torch.linalg.inv(w2c) for w2c in gt_w2c_list]
        
        # Calculate ATE RMSE using c2w (camera center positions)
        ate_rmse_unaligned, ate_rmse_aligned = evaluate_ate(valid_gt_c2w_list, latest_est_c2w_list, return_aligned=True)
        ...
        # Compute detailed statistics for both unaligned and aligned
        gt_traj_pts = [valid_gt_c2w_list[idx][:3,3] for idx in range(len(valid_gt_c2w_list))]
        est_traj_pts = [latest_est_c2w_list[idx][:3,3] for idx in range(len(latest_est_c2w_list))]
        gt_traj_pts = torch.stack(gt_traj_pts).detach().cpu().numpy()
        est_traj_pts = torch.stack(est_traj_pts).detach().cpu().numpy()
        ...
        # Save ATE metrics to JSON files
        import json
        with open(os.path.join(eval_dir, "ate.json"), "w") as f:
            json.dump(ate_unaligned_dict, f, indent=2)
        with open(os.path.join(eval_dir, "ate_aligned.json"), "w") as f:
        ...
        print(f"  Min:        {ate_unaligned_dict['min']*100:.2f} cm")
        print(f"  Max:        {ate_unaligned_dict['max']*100:.2f} cm")
        print(f"\nWith Alignment:")
        ...
        print(f"  Min:        {ate_aligned_dict['min']*100:.2f} cm")
        print(f"  Max:        {ate_aligned_dict['max']*100:.2f} cm")
        print("="*60)
        
        # Use aligned ATE for backward compatibility in plotting
        ate_rmse = ate_rmse_aligned
        
        if wandb_run is not None:
            wandb_run.log({"Final Stats/Avg ATE RMSE (unaligned)": ate_rmse_unaligned,
                        "Final Stats/Avg ATE RMSE (aligned)": ate_rmse_aligned,
                        "Final Stats/step": 1})
        ...
        traceback.print_exc()
        ate_rmse = 100.0
        ate_rmse_aligned = 100.0
        ate_rmse_unaligned = 100.0
